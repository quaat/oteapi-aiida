...
@task(help={"version": "oteapi-aiida version to set"})
def setver(_, version=""):
    """Sets the oteapi-aiida version."""
    match = re.fullmatch(
        (
    ...
    update_file(
        TOP_DIR / "oteapi_aiida" / "__init__.py",
...
def create_api_reference_docs(context, pre_clean=False, pre_commit=False):
    ...
    package_dir = TOP_DIR / "oteapi_aiida"
...
def create_docs_index(_):
    ...
    replacement_mapping = [
        ...
        (
            "(docker-compose.yml)",
            "(https://github.com/quaat/oteapi-aiida/blob/main/docker-compose.yml)",  # pylint: disable=line-too-long
        ),
        (
            "(setup.cfg)",
            "(https://github.com/quaat/oteapi-aiida/blob/main/setup.cfg)",  # pylint: disable=line-too-long
        ),
    ]
